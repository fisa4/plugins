	...
	if (isset($_POST['bridge_id']) && isset($_POST['bridge_ipaddress']) && isset($_POST['bridge_key'])) {
		...
		if (!$error) {
			...
			if ($bridgeID === '-1') { // New Remote Bridge
				...
				try {
					exec_query(
						$query, array($_SESSION['user_id'], $serverIpaddress, $bridgeKey, 'toadd')
					);
				...
				$stmt = exec_query(
					$query,
					array($serverIpaddress, $bridgeKey, 'tochange', $bridgeID, $_SESSION['user_id'])
				);
...
function bridge_deleteKEY()
{
	if (isset($_REQUEST['bridge_id'])) {
		...
		$stmt = exec_query($query, array('todelete', $bridgeID, $_SESSION['user_id']));
...
function bridge_generatePage($tpl)
{
	...
	if ($stmt->rowCount()) {
		foreach ($bridgelists as $bridgeID => $bridgeData) {
			...
			if ($bridgeData['bridge_status'] == 'ok') {
				$tpl->assign(
					array(
